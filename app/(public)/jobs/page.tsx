...
function translateWorkMode(value: WorkMode | null, t: ReturnType<typeof useTranslations>) {
  ...
  const map: Record<WorkMode, string> = {
    remote: t("jobBoard.values.workMode.remote"),
    hybrid: t("jobBoard.values.workMode.hybrid"),
    onsite: t("jobBoard.values.workMode.onsite"),
  };
  return map[value];
}
...
function translateSalaryPeriod(value: SalaryPeriod | null, t: ReturnType<typeof useTranslations>) {
  ...
  const map: Record<SalaryPeriod, string> = {
    hour: t("jobBoard.values.salaryPeriod.hour"),
    month: t("jobBoard.values.salaryPeriod.month"),
    year: t("jobBoard.values.salaryPeriod.year"),
  };
  return map[value];
}
...
function translateContractType(value: ContractType | null, t: ReturnType<typeof useTranslations>) {
  ...
  const map: Record<ContractType, string> = {
    b2b: t("jobBoard.values.contractType.b2b"),
    employment: t("jobBoard.values.contractType.employment"),
    contract: t("jobBoard.values.contractType.contract"),
    internship: t("jobBoard.values.contractType.internship"),
...
export default function PublicJobsPage() {
  ...
  const jobs = useMemo(() => {
    ...
    return baseJobs.filter((job) => {
      ...
      if (locationNeedle && !locationLabel.includes(locationNeedle)) {
        return false;
      }

      if (workModeFilter && job.work_mode !== workModeFilter) {
        return false;
      }

      if (contractFilter && job.contract_type !== contractFilter) {
        return false;
      }

      if (minSalary !== null && (job.salary_max ?? job.salary_min ?? 0) < minSalary) {
        return false;
      }

      if (maxSalary !== null && (job.salary_min ?? job.salary_max ?? Number.MAX_SAFE_INTEGER) > maxSalary) {
        return false;
      }
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder={t("jobBoard.filters.locationPlaceholder")}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) => setWorkModeFilter(event.target.value as WorkMode | "")}
                    className="w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) => setContractFilter(event.target.value as ContractType | "")}
                    className="w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                  >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      ...
                      placeholder={t("jobBoard.filters.salaryMin")}
                      className="w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                    />
                    <input
                      type="number"
                      ...
                      placeholder={t("jobBoard.filters.salaryMax")}
                      className="w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                    />
                  </div>
                </div>
