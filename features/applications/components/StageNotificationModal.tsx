...
export function StageNotificationModal({
  count = 1, isPending = false, onClose, onSendEmail, onSkipEmail,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-border bg-card shadow-2xl animate-fade-in">
        ...
        <div className="flex flex-col gap-3 p-5">
          <Button onClick={onSendEmail} disabled={isPending}>
            {isPending ? tc("saving") : t("sendEmail")}
          </Button>
          <Button variant="outline" onClick={onSkipEmail} disabled={isPending}>
            {t("withoutEmail")}
          </Button>
          <button
            type="button"
