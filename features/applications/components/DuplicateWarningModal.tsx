...
export function DuplicateWarningModal({
  matches, isPending = false, onClose, onContinue, onReuse,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl animate-fade-in">
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                    <span className="inline-flex items-center gap-1"><Mail className="h-3.5 w-3.5" />{match.email}</span>
                    {match.phone && <span className="inline-flex items-center gap-1"><Phone className="h-3.5 w-3.5" />{match.phone}</span>}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {match.match_reasons.map((reason) => (
                      <span key={reason} className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">
                ...
                <Button variant="outline" onClick={() => onReuse(match)} disabled={isPending} className="gap-2">
                  {t("reuse")}<ArrowRight className="h-4 w-4" />
                </Button>
              </div>
            </div>
          ))}
        ...
        <div className="flex flex-col gap-3 border-t border-border p-5 sm:flex-row">
          <Button onClick={onContinue} disabled={isPending} className="flex-1">
