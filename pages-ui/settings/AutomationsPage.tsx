...
import {
  ...
  useToggleAutomation, useDeleteAutomation, useEmailTemplates,
...
const selectClass = "w-full px-3.5 py-2.5 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring";

function RuleModal({ rule, onClose }: { rule?: AutomationRule; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.template_id) { toast.error(tc("error")); return; }
    const payload = { name: form.name, trigger_type: form.trigger_type, trigger_value: form.trigger_value || null, template_id: form.template_id || null, is_active: form.is_active };
    try {
      if (isEditing) { await updateMutation.mutateAsync(payload); toast.success(t("updated")); }
      else { await createMutation.mutateAsync(payload); toast.success(t("created")); }
      onClose();
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-border rounded-xl w-full max-w-lg shadow-2xl animate-fade-in">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h3 className="font-display font-semibold text-foreground">{isEditing ? t("edit") : t("new")}</h3>
          <button onClick={onClose} className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all"><X className="w-4 h-4" /></button>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <select value={form.trigger_type} onChange={(e) => setForm((f) => ({ ...f, trigger_type: e.target.value, trigger_value: "" }))} className={selectClass}>
              {TRIGGER_TYPES.map((type) => <option key={type} value={type}>{t(`triggers.${type}` as never)}</option>)}
            </select>
          </div>
        ...
        <div className="flex gap-3 p-5 pt-0">
          ...
          <button onClick={onClose} className="px-4 py-2.5 border border-border text-foreground rounded-lg text-sm hover:bg-muted transition-all">{tc("cancel")}</button>
        </div>
      </div>
    </div>
  );
...
export function AutomationsPage() {
  ...
  const handleToggle = async (id: string) => {
    try { await toggleMutation.mutateAsync(id); } catch { toast.error(tc("error")); }
  };

  const handleDelete = async (id: string) => {
    if (!confirm(tc("confirm"))) return;
    try { await deleteMutation.mutateAsync(id); toast.success(t("deleted")); }
    catch { toast.error(tc("error")); }
  };
            ...
            {rules?.map((rule, i) => (
              <div key={rule.id}
                ...
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${rule.is_active ? "bg-primary/10" : "bg-muted"}`}>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{t(`triggers.${rule.trigger_type}` as never)}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        rule.is_active ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-muted text-muted-foreground"
                      }`}>
                ...
                <div className="flex items-center gap-1 ml-4">
                  <button onClick={() => handleToggle(rule.id)} title={rule.is_active ? t("disable") : t("enable")}
                    className={`p-2 rounded-lg transition-all ${rule.is_active ? "text-green-500 hover:bg-green-500/10" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}>
                    <Power className="w-4 h-4" />
                  </button>
                  <button onClick={() => setEditRule(rule)} className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all"><Pencil className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(rule.id)} className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            ))}
          </div>
