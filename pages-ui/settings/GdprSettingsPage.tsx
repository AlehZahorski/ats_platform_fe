...
import { useConsents, useCreateConsent, useUpdateConsent, useDeleteConsent, useCleanupExpired } from "@/services/queries/gdpr.queries";
import type { Consent } from "@/services/api/gdpr";

const LANGUAGES = ["en", "pl"];

function ConsentModal({ consent, onClose }: { consent?: Consent; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!form.name.trim() || !form.content.trim()) { toast.error(tc("error")); return; }
    try {
      if (isEditing) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-border rounded-xl w-full max-w-lg shadow-2xl animate-fade-in">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h3 className="font-display font-semibold text-foreground">{isEditing ? tc("edit") : t("new")}</h3>
...
export function GdprSettingsPage() {
  ...
  const handleToggle = async (consent: Consent) => {
    try { await updateMutation.mutateAsync({ id: consent.id, data: { is_active: !consent.is_active } }); }
    catch { toast.error(tc("error")); }
  };
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-semibold text-foreground flex items-center gap-2">
              <Shield className="w-4 h-4 text-primary" /> {t("title")}
            </h2>
          ...
          {isLoading ? (
            <div className="space-y-3">{[...Array(2)].map((_, i) => <div key={i} className="h-20 bg-card border border-border rounded-xl animate-pulse" />)}</div>
          ) : consents?.length === 0 ? (
            ...
            <div className="space-y-3">
              {consents?.map((consent, i) => (
                <div key={consent.id}
                  className={`bg-card border rounded-xl p-5 transition-all animate-fade-in ${consent.is_active ? "border-border" : "border-border/50 opacity-60"}`}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => handleToggle(consent)}
                        className={`p-2 rounded-lg transition-all ${consent.is_active ? "text-green-500 hover:bg-green-500/10" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}>
                        <Power className="w-4 h-4" />
                      </button>
                      <button onClick={() => setEditConsent(consent)} className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(consent.id)} className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all">
