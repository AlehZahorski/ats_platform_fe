...
import {
  ...
  useDeleteEmailTemplate, usePreviewEmailTemplate,
} from "@/services/queries/email-automation.queries";
import type { EmailTemplate } from "@/services/api/email-templates";

const TEMPLATE_TYPES = ["application_received", "interview_invite", "rejection", "offer", "custom"];
const LANGUAGES = ["en", "pl"];
const VARIABLES = ["{{candidate_name}}", "{{job_title}}", "{{company_name}}", "{{stage_name}}", "{{tracking_url}}", "{{interview_date}}", "{{interview_url}}"];
...
function TemplateModal({ template, onClose }: { template?: EmailTemplate; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.subject || !form.body) { toast.error(tc("error")); return; }
    try {
      if (isEditing) { await updateMutation.mutateAsync(form); toast.success(t("updated")); }
      else { await createMutation.mutateAsync(form); toast.success(t("created")); }
      onClose();
...
function PreviewModal({ template, onClose }: { template: EmailTemplate; onClose: () => void }) {
  ...
  useEffect(() => {
    preview.mutateAsync(undefined).then(setPreviewData).catch(() => {});
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{t(`types.${tpl.type}` as never)}</span>
                      <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full uppercase">{tpl.language}</span>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1 truncate max-w-md">{tpl.subject}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1 ml-4">
                  <button onClick={() => setPreviewTemplate(tpl)} className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all"><Eye className="w-4 h-4" /></button>
                  <button onClick={() => setEditTemplate(tpl)} className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all"><Pencil className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(tpl.id)} className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            ))}
          </div>
