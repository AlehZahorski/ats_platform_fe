...
import {
  ArrowLeft, ExternalLink, Image as ImageIcon, LoaderCircle, Save, Trash2, Upload, X,
} from "lucide-react";
...
import {
  ...
  useRemoveCompanyArticleCover,
  useToggleCompanyPublish,
  useUpdateCompanyArticle,
  useUploadCompanyArticleCover,
} from "@/services/queries/company-articles.queries";
import { ArticleContentEditor } from "@/pages-ui/admin/ArticleContentEditor";
...
export function CompanyArticleEditorPage({ articleId }: Props) {
  const router = useRouter();
  const isNew = articleId === null;

  const { data: me } = useMe();
  const { data: article, isLoading } = useCompanyArticle(articleId);
  const create = useCreateCompanyArticle();
  const update = useUpdateCompanyArticle(articleId ?? "");
  ...
  // For new articles, prefill the byline from the logged-in user — saves
  ...
  const onSave = async () => {
    ...
    const payload = {
      ...
      author_name: draft.author_name.trim() || "Zespół firmy",
  ...
  const onDelete = async () => {
    if (!articleId) return;
    if (!window.confirm(`Usunąć artykuł „${draft.title}"?`)) return;
    try {
  ...
  return (
    <>
      <Topbar title={isNew ? "Nowy artykuł" : (article?.title || "Edycja artykułu")} />
      <Header
        article={article}
        isNew={isNew}
        title={draft.title}
      ...
      <div className="flex-1 px-6 py-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-5 min-w-0">
          <section className="rounded-2xl border border-border bg-card p-5 space-y-3">
            <Field label="Tytuł">
              <input
                ...
                placeholder="np. Jak budujemy zespół backend w naszej firmie"
                className={inputCls}
              />
            </Field>
            <Field label="Adres URL (slug)">
              <div className="flex items-center gap-1">
                <span className="text-xs text-muted-foreground shrink-0 px-2">/firmy-pisza/</span>
                <input
                  value={draft.slug}
                  onChange={(e) => {
                  ...
                  placeholder="jak-budujemy-zespol"
                  className={inputCls}
                />
              </div>
            </Field>
          ...
          <section className="rounded-2xl border border-border bg-card p-5">
            ...
            <ArticleContentEditor
              ...
              placeholder="Podziel się historią — case study, kulisy projektu, kultura zespołu…"
            />
          </section>
        </div>
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-2xl border border-border bg-card p-5 space-y-3">
            ...
            <Field label="Autor — rola w firmie">
              <input
          ...
          <section className="rounded-2xl border border-border bg-card p-5 space-y-3">
            ...
            {isNew && (
              <p className="text-[11px] text-muted-foreground/70 leading-tight">
                {draft.is_published
                  ? 'Po kliknięciu „Utwórz" artykuł od razu pojawi się na /firmy-pisza.'
                  : "Zapisany jako szkic — widoczny tylko w waszym panelu."}
              </p>
            )}
          </section>
        </aside>
...
// ─────────────────────────────────────────────────────────────────────
// Header — analogous to admin editor header (sans Featured badge).
// ─────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────
// Cover uploader — uses company-scoped mutations.
// ─────────────────────────────────────────────────────────────────────

function CoverUploader({ article, disabled }: { article: AdminArticle | undefined; disabled: boolean }) {
  const upload = useUploadCompanyArticleCover(article?.id ?? "");
  const remove = useRemoveCompanyArticleCover(article?.id ?? "");
...
function RowFlex({ label, children }: { label: string; children: React.ReactNode }) {
  return (
