...
import { useTimeToHireReport, usePipelineReport, useSourcesReport } from "@/services/queries/tasks-reports.queries";
...
function Bar({ label, count, percentage, color = "bg-primary" }: { label: string; count: number; percentage: number; color?: string }) {
  return (
    <div className="space-y-1">
      ...
      <div className="h-2 bg-muted rounded-full overflow-hidden">
        <div className={`h-full ${color} rounded-full transition-all duration-500`} style={{ width: `${Math.max(percentage, 2)}%` }} />
      </div>
    </div>
  );
}

const SOURCE_COLORS = ["bg-primary", "bg-blue-500", "bg-teal-500", "bg-amber-500", "bg-purple-500", "bg-pink-500"];
    ...
    <div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          {pipeLoading ? (
            <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-8 bg-muted/40 rounded animate-pulse" />)}</div>
          ) : pipeline?.stages.length === 0 ? (
            <p className="text-muted-foreground text-sm">{t("noData")}</p>
          ) : (
            <div className="space-y-4">
              {pipeline?.stages.map((stage) => (
                <Bar key={stage.stage_name} label={stage.stage_name} count={stage.count} percentage={stage.percentage} />
              ))}
              <p className="text-xs text-muted-foreground pt-2 border-t border-border">Total: {pipeline?.total} {t("candidates")}</p>
            </div>
          )}
        </div>
          ...
          {srcLoading ? (
            ...
            <div className="space-y-4">
              {sources?.sources.map((src, i) => (
                <Bar key={src.source} label={src.source} count={src.count} percentage={src.percentage} color={SOURCE_COLORS[i % SOURCE_COLORS.length]} />
              ))}
              <p className="text-xs text-muted-foreground pt-2 border-t border-border">Total: {sources?.total} {t("candidates")}</p>
            </div>
          )}
        </div>
      </div>
