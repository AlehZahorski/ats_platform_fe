...
import { Search, Users, ChevronRight, CheckSquare, Square, Minus, X, GitBranch, Tag, XCircle } from "lucide-react";
import { Topbar } from "@/shared/layout/Topbar";
import { StageNotificationModal } from "@/features/applications/components/StageNotificationModal";
import { useApplications, useBulkAction, usePipelineStages } from "@/services/queries";
import { useTags } from "@/services/queries/tags.queries";
...
function BulkToolbar({
  selectedIds, onClear, onAction, onRequestStageNotification, onRequestRejectNotification,
}: {
  selectedIds: string[];
  onClear: () => void;
  ...
  const translateStageName = (stageName: string) => {
    ...
    return knownKeys.includes(key) ? tStages(key as never) : stageName;
  };
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30 animate-fade-in">
      <div className="bg-card border border-border rounded-2xl shadow-2xl px-4 py-3 flex items-center gap-3 min-w-[400px]">
        <div className="flex items-center gap-2 pr-3 border-r border-border">
          ...
          <span className="text-sm font-medium text-foreground whitespace-nowrap">{t("selected", { count: selectedIds.length })}</span>
        </div>

        <div className="relative">
          <button onClick={() => { setShowStages(!showStages); setShowTags(false); }}
            ...
            <GitBranch className="w-3.5 h-3.5" /> {t("changeStage")}
          </button>
          {showStages && (
            <div className="absolute bottom-full mb-2 left-0 bg-card border border-border rounded-xl shadow-lg overflow-hidden min-w-[160px]">
              {stages?.map((stage) => (
                <button key={stage.id} onClick={() => { setShowStages(false); onRequestStageNotification(stage.id); }}
        ...
        <div className="relative">
          <button onClick={() => { setShowTags(!showTags); setShowStages(false); }}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted rounded-lg transition-all">
            <Tag className="w-3.5 h-3.5" /> {t("addTag")}
          </button>
          {showTags && (
            <div className="absolute bottom-full mb-2 left-0 bg-card border border-border rounded-xl shadow-lg overflow-hidden min-w-[160px] max-h-48 overflow-y-auto">
              {tags?.length === 0 && <p className="px-3.5 py-2.5 text-sm text-muted-foreground">{tApplications("noTags")}</p>}
              {tags?.map((tag) => (
                <button key={tag.id} onClick={() => { setShowTags(false); onAction("tag", { tag_id: tag.id }); }}
                  className="w-full text-left px-3.5 py-2.5 text-sm text-foreground hover:bg-muted transition-colors">
                  {tag.name}
                </button>
              ))}
        ...
        <button onClick={onRequestRejectNotification}
        ...
        <button onClick={onClear} className="ml-auto p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all">
          <X className="w-4 h-4" />
        </button>
      </div>
    </div>
...
export function ApplicationsPage() {
  ...
  const { data: stages } = usePipelineStages();
  const { data, isLoading } = useApplications({ search: search || undefined, stage_id: stageFilter || undefined });
  ...
  const handleBulkAction = async (action: "stage_change" | "reject" | "tag", payload: Record<string, string | boolean>) => {
    try {
      const result = await bulkMutation.mutateAsync({ application_ids: Array.from(selectedIds), action, payload });
      toast.success(result.failed > 0 ? t("bulkUpdatedWithFailed", { updated: result.updated, failed: result.failed }) : t("bulkUpdated", { updated: result.updated }));
      setSelectedIds(new Set());
    } catch { toast.error(tc("error")); }
    finally { setPendingNotificationAction(null); }
  };
    ...
    <div>
      ...
      <div className="p-6 space-y-5">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select value={stageFilter} onChange={(e) => setStageFilter(e.target.value)}
            ...
            {stages?.map((stage) => <option key={stage.id} value={stage.id}>{translateStageName(stage.name)}</option>)}
          </select>
        </div>
            ...
            {data?.items.map((application, index) => {
              ...
              return (
                <div key={application.id}
                  ...
                  <button onClick={() => toggleOne(application.id)} className="shrink-0 text-muted-foreground hover:text-primary transition-colors">
                  ...
                  <Link href={`/dashboard/applications/${application.id}`} className="flex-1 flex items-center gap-4 min-w-0">
                    <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                      <span className="text-xs font-bold text-primary">{application.first_name[0]}{application.last_name[0]}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{application.first_name} {application.last_name}</p>
                      <p className="text-xs text-muted-foreground truncate">{application.email}</p>
                    </div>
                    <div className="hidden sm:block shrink-0">
                      {application.stage ? (
                        <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STAGE_COLORS[application.stage.name] ?? "bg-muted text-muted-foreground"}`}>
                          {translateStageName(application.stage.name)}
                        </span>
                      ) : <span className="text-xs text-muted-foreground">—</span>}
                    </div>
                    <p className="hidden md:block text-xs text-muted-foreground shrink-0">{formatRelative(application.created_at)}</p>
      ...
      {selectedIds.size > 0 && (
        <BulkToolbar
          ...
          onRequestStageNotification={(stageId) => setPendingNotificationAction({ action: "stage_change", payload: { stage_id: stageId } })}
          onRequestRejectNotification={() => setPendingNotificationAction({ action: "reject", payload: {} })}
        />
      )}
      ...
      {pendingNotificationAction && (
        <StageNotificationModal
          ...
          onSendEmail={() => handleBulkAction(pendingNotificationAction.action, { ...pendingNotificationAction.payload, notify_candidate: true })}
          onSkipEmail={() => handleBulkAction(pendingNotificationAction.action, { ...pendingNotificationAction.payload, notify_candidate: false })}
        />
      )}
    </div>
  );
