...
import { JobMatchSection } from "@/features/applications/components/JobMatchSection";
import { InterviewsSection } from "@/features/interviews/components/InterviewsSection";
import { GdprSection } from "@/features/gdpr/components/GdprSection";
import { HiringManagerReviewSection } from "@/features/reviews/components/HiringManagerReviewSection";
...
export function ApplicationDetailPage({ applicationId }: ApplicationDetailPageProps) {
  ...
  useEffect(() => {
    if (app?.scores && app.scores.length > 0) {
      ...
      setScores({ communication: latest.communication ?? 3, technical: latest.technical ?? 3, culture_fit: latest.culture_fit ?? 3 });
    }
  }, [app]);

  const handleAddNote = async () => {
    if (!noteContent.trim()) return;
    try { await addNote.mutateAsync({ content: noteContent, visible_to_candidate: false }); setNoteContent(""); toast.success(tn("added")); }
    catch { toast.error(tn("failed")); }
  };

  const handleStageChange = async (stage_id: string, notifyCandidate: boolean) => {
    try { await updateStage.mutateAsync({ id: applicationId, stage_id, notify_candidate: notifyCandidate }); toast.success(t("stageUpdated")); setPendingStageId(null); }
    catch (error: unknown) { toast.error(getApiErrorMessage(error) ?? t("stageUpdateFailed")); }
  };

  const handleScore = async () => {
    try { await scoreApp.mutateAsync(scores); toast.success(ts("saved")); }
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-card border border-border rounded-xl p-6 animate-fade-in">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><Mail className="w-4 h-4" /> {app.email}</div>
              {app.phone && <div className="flex items-center gap-2 text-sm text-muted-foreground"><Phone className="w-4 h-4" /> {app.phone}</div>}
              {app.cv_url && (
              ...
              <div className="space-y-4">
                {(app.answers ?? []).map((ans) => (
                  <div key={ans.id} className="border-b border-border pb-3 last:border-0">
                    <p className="text-xs font-medium text-muted-foreground mb-1">{ans.field_label ?? `Field ${ans.field_id.slice(0, 8)}`}</p>
              ...
              {stages?.map((stage) => (
                <button key={stage.id} onClick={() => setPendingStageId(stage.id)}
                  className={`w-full text-left px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                    app.stage?.id === stage.id ? "bg-primary text-primary-foreground" : "text-foreground hover:bg-muted border border-border"
                  }`}>
                  {stage.name}
                </button>
              ))}
