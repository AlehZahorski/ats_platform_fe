...
import { Plus, Tag as TagIcon, Trash2, Hash } from "lucide-react";
import { Topbar } from "@/shared/layout/Topbar";
import { useTags, useCreateTag } from "@/services/queries/tags.queries";
...
function TagCard({ tag, index, onDeleted }: { tag: Tag; index: number; onDeleted: () => void }) {
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <h3 className="font-semibold text-foreground text-sm mb-3 flex items-center gap-2">
            <Hash className="w-4 h-4 text-primary" /> {t("new")}
          </h3>
          <div className="flex gap-3">
            <input
              value={newTag} onChange={(e) => setNewTag(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleCreate()}
              placeholder="e.g. senior, react, recommended..." maxLength={30}
            ...
            />
            <button onClick={handleCreate} disabled={!newTag.trim() || createMutation.isPending}
        ...
        <div className="space-y-3">
          ...
          {isLoading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {[...Array(8)].map((_, i) => <div key={i} className="h-14 bg-card border border-border rounded-xl animate-pulse" />)}
            </div>
          ) : filtered?.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {filtered?.map((tag, i) => (
                <TagCard key={tag.id} tag={tag} index={i} onDeleted={() => qc.invalidateQueries({ queryKey: ["tags"] })} />
              ))}
            </div>
          )}
        </div>
