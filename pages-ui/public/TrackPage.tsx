...
export function TrackPage({ token }: TrackPageProps) {
        ...
        <div className="bg-card border border-border rounded-xl p-8 shadow-lg animate-fade-in-delay-1">
          <div className="text-center mb-8">
            <span className={`inline-block px-6 py-2.5 rounded-full font-semibold text-lg ${
              ...
              : isHired ? "bg-green-500/10 text-green-500"
                  ...
                  <div key={stageName} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all ${
                        ...
                        {isCompleted ? <CheckCircle className="w-4 h-4" /> : isCurrent ? <Clock className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
                      </div>
                    ...
                    <div className="pb-6 pt-1">
                      <p className={`font-medium text-sm ${isCurrent ? "text-primary" : isCompleted ? "text-foreground" : "text-muted-foreground"}`}>
                        ...
                        {isCurrent && (
                          <span className="ml-2 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{t("current")}</span>
                        )}
                      </p>
