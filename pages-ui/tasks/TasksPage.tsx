...
import { Plus, CheckSquare, Square, Trash2, Clock, X } from "lucide-react";
...
const inputClass = "w-full px-3.5 py-2.5 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring";

function CreateTaskForm({ onClose }: { onClose: () => void }) {
  const t = useTranslations("tasks");
  const tc = useTranslations("common");
  const createMutation = useCreateTask();
  const [form, setForm] = useState({ title: "", description: "", type: "", due_date: "" });
    ...
    <div className="bg-card border border-border rounded-xl p-5 animate-fade-in">
      ...
      <div className="space-y-3">
        <div>
          ...
          <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} autoFocus placeholder="e.g. Follow up with candidate" className={inputClass} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))} className={inputClass}>
              ...
              {TASK_TYPES.map((type) => <option key={type} value={type}>{t(`types.${type}` as never)}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-foreground mb-1">{t("dueDate")}</label>
            <input type="datetime-local" value={form.due_date} onChange={(e) => setForm((f) => ({ ...f, due_date: e.target.value }))} className={inputClass} />
          </div>
        </div>
        <div>
...
function TaskCard({ task }: { task: Task }) {
  ...
  const handleToggle = async () => {
    try { await updateMutation.mutateAsync({ id: task.id, data: { completed: !task.completed } }); }
    catch { toast.error(tc("error")); }
  };

  const handleDelete = async () => {
    if (!confirm(tc("confirm"))) return;
    try { await deleteMutation.mutateAsync(task.id); toast.success(t("deleted")); }
    catch { toast.error(tc("error")); }
  };
    ...
    <div className={`bg-card border rounded-xl p-4 flex items-start gap-3 transition-all animate-fade-in ${
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1.5">
          {task.type && (
            <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{t(`types.${task.type}` as never)}</span>
          )}
          {task.due_date && (
            <span className={`flex items-center gap-1 text-xs ${isOverdue ? "text-destructive" : "text-muted-foreground"}`}>
              <Clock className="w-3 h-3" /> {formatDate(task.due_date)}{isOverdue && " — overdue"}
            </span>
          )}
          {task.assignee && <span className="text-xs text-muted-foreground">👤 {task.assignee.email}</span>}
        </div>
      </div>
...
export function TasksPage() {
  ...
  const { data: tasks, isLoading } = useTasks(
    filter === "pending" ? { completed: false } : filter === "completed" ? { completed: true } : undefined
  );
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-5">
        ...
        {isLoading ? (
          <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="h-16 bg-card border border-border rounded-xl animate-pulse" />)}</div>
        ) : tasks?.length === 0 ? (
